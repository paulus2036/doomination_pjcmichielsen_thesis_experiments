...
class Runner(EventLoopObject, Configurable):
    def __init__(self, cfg, unique_name=None):
        ...
        # for continual learning setting, we keep track of which env we are in
    ...
    def _report_experiment_summaries(self):
        ...
        for policy_id, env_steps in self.env_steps.items():
            ...
            env_index = self.current_env_id % len(self.cfg.envs)
            writer.add_scalar("task_id", env_index, env_steps)
            ...
            if self.cfg.with_wandb and 'heatmap' in self.policy_avg_stats and self.cfg.log_heatmap:
    ...
    def log_new_videos_to_wandb(self):
        ...
        env_name = self.cfg.envs[self.current_env_id % len(self.cfg.envs)]
        video_path = video_dir(self.cfg, env_name, self.cfg.video_dir)
        ...
        for step_number, video_file in sorted(new_videos):
            video_file_path = join(video_path, video_file)
            video_tag = f"video/{self.env_info.name}_step_{step_number}"

            if not is_file_locked(video_file_path):
                # Video has been stored and is ready to be uploaded
                wandb.log({video_tag: wandb.Video(video_file_path, format='mp4')})
    ...
    def _propagate_training_info(self):
        ...
        for policy_id in range(self.cfg.num_policies):
            training_info[policy_id] = dict(
                ...
                current_env_id=self.current_env_id % len(self.cfg.envs),
    ...
    def _should_go_to_next_env(self):
        next_switch_step = (self.current_env_id + 1) * self.cfg.env_steps_per_env
        return len(self.env_steps) > 0 and all(s > next_switch_step for s in self.env_steps.values())

    def _go_to_next_env(self):
        self.current_env_id = (self.current_env_id + 1) % len(self.cfg.envs)
        self._propagate_training_info()  # emit the new env id to the sampler & rollout workers right away
    ...
    def _after_training_iteration(self, training_iteration_since_resume: int):
        ...
        if self._should_go_to_next_env():
            self._go_to_next_env()
