...
class BatchedVectorEnvRunner(VectorEnvRunner):
    ...
    def update_env_id(self, new_env_id: int):
        """
        Update the environment id for all environments in the vector.
        """
        if hasattr(self.vec_env, "switch_to_next_env"):
            self.vec_env.switch_to_next_env(new_env_id)
