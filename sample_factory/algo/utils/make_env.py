...
class ContinualEnv(gym.Env):
    """
    Wraps a list of env IDs or specs and runs them one after the other.
    When env i is done, move on to env i+1. Once we exhaust the list, cycle back to the first env.

    Now with the ability to forward unknown attributes to the current active env.
    """

    def __init__(self, env_names: List[str], cfg, env_config, render_mode):
        super().__init__()
        self.env_names = env_names
        self.cfg = cfg
        self._curr_env_idx = 0
        self._switch_to_do = False
        self.current_env: Optional[gym.Env] = None
        self.env_config = env_config
        self.render_mode = render_mode

        # Create the first environment
        self._load_env(self._curr_env_idx)

    def _load_env(self, idx: int):
        self.current_env = create_env(self.env_names[idx], cfg=self.cfg, env_config=self.env_config,
    ...
    def step(self, action):
        obs, rew, done, trunc, info = self.current_env.step(action)

        if done or trunc:
            if self._switch_to_do:
                self._switch_env_after_episode_done()
                self._switch_to_do = False
                info["continual_env_switched_to"] = self._curr_env_idx

        return obs, rew, done, trunc, info

    def switch_to_next_env(self, new_env_id: int):
        self._curr_env_idx = new_env_id % len(self.env_names)  # loop around if needed
        self._switch_to_do = True

    def _switch_env_after_episode_done(self):
        self.close()
        self._load_env(self._curr_env_idx)
        print(f"ContinualEnv: loaded new task {self._curr_env_idx} - {self.env_names[self._curr_env_idx]}")
    ...
    # ---------------------------------------------------------------------
    # FORWARD UNRECOGNIZED ATTRIBUTE ACCESSES TO THE CURRENT ENV
    # ---------------------------------------------------------------------

    def __getattr__(self, name):
        """
        If 'name' is not found on ContinualEnv itself, forward to the current_env.
        This allows e.g. env.game, env.initialized, env.show_automap, etc.
        """
        # Avoid recursion if current_env is None or we're asking for special __-attributes
    ...
    def __setattr__(self, name, value):
        """
        If we are setting one of our own known attributes (env_names, cfg, _curr_env_idx, current_env, etc.),
        do so normally. Otherwise, try setting it on the current_env.
        """
        # If it's one of our local attributes, set locally
        if name in ("env_names", "cfg", "_curr_env_idx", "current_env",
                    "observation_space", "action_space", "_switch_to_do"):
            super().__setattr__(name, value)
        else:
            # Otherwise forward to the current_env (if we have one)
            if self.current_env is not None and hasattr(self.current_env, name):
                setattr(self.current_env, name, value)
            else:
                # If the current_env doesn't exist or doesn't have this attr, set it locally
                super().__setattr__(name, value)
...
def make_env_func_batched(cfg, env_config, render_mode: Optional[str] = None) -> BatchedVecEnv:
    ...
    """
    if len(cfg.envs) > 1:
        # We have multiple environment names in cfg.envs. Create the chain.
        env = ContinualEnv(cfg.envs, cfg, env_config, render_mode=render_mode)
    else:
        # Original single-env logic (unchanged)
        env = create_env(cfg.envs[0], cfg=cfg, env_config=env_config, render_mode=render_mode)
