...
def add_basic_cli_args(p: ArgumentParser):
    ...
    p.add_argument("--envs", type=str, nargs='+', default=["volcanic_venture", "remedy_rush", "collateral_damage", "precipice_plunge", "detonators_dilemma", "armament_burden"], help="Name of the environment to use")
...
def add_rl_args(p: ArgumentParser):
    ...
    p.add_argument(
        ...
        default=False,
        type=str2bool,
        help="Whether to randomize and shuffle minibatches between iterations (this is a slow operation when "
             "batches are large, disabling this increases learner throughput when training with multiple epochs/minibatches per epoch)",
    )
    ...
    p.add_argument(
        ...
        " For many tasks the first (epsilon) distribution is preferable because we keep some "
    ...
    p.add_argument(
        ...
        help=(
            ...
            "after the last minibatch/epoch significantly deviates from this threshold, lr is appropriately"
    ...
    p.add_argument(
        ...
        default=True,
        type=str2bool,
        help="Whether to multiply training steps by frameskip when recording summaries, FPS, etc. "
             "When this flag is set to True, x-axis for all summaries corresponds to the total number of simulated steps, "
             "i.e. with frameskip=4 the x-axis value of 4 million will correspond to 1 million frames observed by the policy.",
    )
    ...
    p.add_argument(
        ...
        default=-1,
        type=int,
        help="Used in continual learning setting. Number of env_steps per environment, before switching to the next environment."
             "If set to -1, the environment will be switched after ceil(train_for_env_steps / len(envs)) steps."
    )

    # model saving
    p.add_argument("--save_models", default=False, type=str2bool, help="Whether to save policy checkpoints")
...
def add_model_args(p: ArgumentParser):
    ...
    Model builder automatically detects whether we should use conv or MLP encoder, then we use parameters to specify
...
def add_logging_args(p: ArgumentParser):
    ...
    p.add_argument("--console_log", default="", type=str, help="Path to console output log file")
    p.add_argument(
        "--log_heatmap",
        default=False,
        type=str2bool,
