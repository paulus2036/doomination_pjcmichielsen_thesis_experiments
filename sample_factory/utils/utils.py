...
def experiment_name(cfg):
    return f'{cfg.algo}_level_{cfg.level}_{cfg.constraint}_seed_{cfg.seed}_{cfg.timestamp}'
...
def experiment_dir(cfg, mkdir=True) -> str:
    ...
    experiment = f'{cfg.algo}/Level_{cfg.level}{suffix}'
    experiments_root = experiments_dir(cfg, mkdir)
    return maybe_ensure_dir_exists(join(experiments_root, experiment), mkdir)

def video_dir(cfg, env_name, video_folder, mkdir=False) -> str:
    suffix = f"/{cfg.timestamp}" if cfg.timestamp else ""
    experiment = f'{cfg.algo}/Level_{cfg.level}{suffix}/{video_folder}/{env_name}'
    experiments_root = experiments_dir(cfg, mkdir)
    return maybe_ensure_dir_exists(join(experiments_root, experiment), mkdir)
